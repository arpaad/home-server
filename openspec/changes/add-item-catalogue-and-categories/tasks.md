...
- [x] 2.8 Ship a starter pack as data (`seed_data/catalogue.hu.json`): categories with icon and colour, and everyday entries each categorised; seed both only into an empty table; verify tests that the pack is well-formed, that seeding an empty database links every entry, that seeding twice changes nothing, and that a removed starter category is not reinserted
- [x] 2.9 Cap the catalogue page's unfiltered list and hoist its mutation hooks out of the rows, since a several-hundred-row page mounted four hooks per row; verify an end-to-end test that the page shows at most the cap with a "type to narrow" hint and that the search finds a starter entry
